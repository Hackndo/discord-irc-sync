...
class IRCClient(irc.bot.SingleServerIRCBot):
    def __init__(self, configuration):
        ...
        self.callback = {
            "notification": {"nick_in_use": self.unimplemented},
            "message": {"default": self.h_send_message, "raw": self.h_send_message},
            "user": {
    ...
    def on_welcome(self, server, event):
        ...
        logger.info("[IRC] Logged in as:")
        logger.info("[IRC] %s" % self.h_nickname)
        if self.master_bot is False:
    ...
    def on_kick(self, server, event):
        ...
        if self.h_log_events:
            message = "*%s* has been kicked of the channel (%s)" % (
                event.arguments[0],
                reason,
            )
    ...
    def on_nicknameinuse(self, server, event):
        if self.master_bot is True:
            ...
            logger.info("User already in use on IRC. Killing thread & notifying the discord class")
    ...
    def h_send_to_discord(self, username, content):
        if self.master_bot is True:
            ...
            logger.info("[IRC] %s" % message)

            if content.startswith(self.h_cmd_prefix):
                self.h_send_notification(
                    "message",
                    "raw",
                    self.h_output_cmd.replace(":username:", username),
                    username,
                )
